...
- Abajo a la izquierda ya no hay indicadores de cookies: están en **Archivo → Preferencias → pestaña Cookies** (YouTube, Twitch y Kick, con **Reexportar cookies**). Los menús **Youtube**, **Twitch** y **Kick** siguen mostrando **Sesión …: OK / caducada**. En **Windows** usa Firefox (Chrome, Brave y Edge cifran las cookies). Detalle en [YouTube](youtube.md#cookies) y [uso (Twitch/Kick)](uso.md).
...
Desde **Youtube → Buscar en YouTube** puedes **añadir a la cola**: los vídeos van a **Youtube → Cola de YouTube** (siguiente, quitar, reordenar), no a esta lista. Al terminar el vídeo en curso se reproduce el primero de esa cola. Un vídeo de YouTube suelto (búsqueda, URL o historial) al acabar de verdad ofrece **volver a verlo**; si el stream se corta o se congela a mitad (típico en vídeos largos cuando caduca el enlace), se reintenta reanudar desde ahí (hasta 6 veces) en lugar de quedarse bloqueado. Una playlist, un canal o **Reproducir desde aquí** no pregunta.

**Reproducir → Limpiar lista lateral** hace lo mismo que el botón. No detiene el vídeo en curso. **Reproducir → Vaciar cachés…** borra logos/epg, vídeos YouTube en disco y las cachés en memoria (con confirmación; no toca grabaciones). **Reproducir → Historial** lista IPTV, YouTube, Twitch y Kick (recientes y seguir viendo); **Ver historial…** o **Biblioteca…** abren la [biblioteca unificada](uso.md) (**Favoritos → Biblioteca…**), con filtros por sección y fuente. **Reproducir → Guía EPG → Parrilla…** abre la parrilla; **Desde URL…** / **Desde archivo…** indican el XMLTV. **Reproducir → Preferencias** abre la misma ventana que **Archivo → Preferencias** en la principal.
...
Play/pausa (también **un clic en el vídeo** o `Espacio`), stop, **grabar** (círculo; se pone rojo mientras copia), **anterior** (si van más de 3 s reinicia; si no, canal previo) / **siguiente**, saltos de 2 s, **calidad / audio**, **subtítulos**, volumen, silencio, **PiP**, pantalla completa y mostrar/ocultar la lista (`≡`). Los tooltips de los botones muestran el atajo.
