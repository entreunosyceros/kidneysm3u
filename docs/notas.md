...
- **VLC embebido** (`python-vlc`): decodificación por software (`avcodec-hw=none`) para evitar ruido de VA-API en NVIDIA. En **Preferencias → Modo ligero** se puede permitir GPU en IPTV si VLC lo soporta. Audio en Linux: ALSA en IPTV, Pulse en YouTube cuando hace falta. En Windows y macOS no se fuerza módulo: VLC usa DirectSound/Wasapi o Core Audio.
...
- **Listas grandes**: el filtrado de la ventana principal y la carga/parseo del M3U van en segundo plano (con debounce más largo cuanto más grande sea la lista). La barra lateral agrupa por `group-title`: pestañas si hay pocos grupos, desplegable si hay muchos; un clic entra en la categoría. Sin grupos (o con muchos resultados de filtro), a partir de ~1200 filas (~600 si hay búsqueda activa) solo se dibujan las visibles. Pintar o filtrar una lista enorme en la lateral aún puede notar un momento de carga, pero menos que antes.
- **EPG**: XMLTV (en el M3U o en **Reproducir → Guía EPG**, por URL o archivo). Se asocia por `tvg-id`, `tvg-name` o el nombre del canal frente a `<display-name>`. Hay parrilla, programa actual en la fila y ahora / a continuación al pasar el ratón. La guía se pide en segundo plano (timeout 90 s), se recarga cada 30 min y no se registran las URLs (pueden llevar token). En **modo ligero** (manual o **automático** al superar ~1000 canales o CPU alta) no hay recarga periódica, el tick de actualización pasa a ~5 min, solo se pide EPG del grupo visible y se apagan los logos. Una URL `get.php` o `xmltv.php` se usa tal cual; no se reescribe a otra ruta Xtream. Miniaturas: `tvg-logo` / `<icon>` en `epg_cache/` (no va al git); se pueden apagar en **Preferencias** o en **Guía EPG → Mostrar logos de canal**, o vaciar con **Limpiar caché de logos** en Preferencias.
...
- **Audio y subtítulos**: VLC lista las pistas del stream (típico en IPTV/HLS) en **Calidad / audio**. YouTube: transcripción ASR, pistas del autor y traducción automática (VTT con `tlang`; el json3 traducido suele seguir en inglés). No hay cambio de idioma de audio. Los VTT de YouTube se convierten a SRT para que VLC aplique el estilo (`freetype-rel-fontsize`, paleta de colores, opacidad y contorno; instancia + media). El estilo se guarda en `config.json`; no vale para DVB/PGS de imagen. Al guardar Preferencias se recarga el stream o el canal IPTV en curso.
...
| --- | --- | --- |
| `favoritos.json` | Favoritos del reproductor. **Favoritos → Exportar / Importar** copia esa lista a otro equipo (JSON o M3U) | Se crea al guardar el primer favorito. Si falta, el reproductor usa una lista vacía |
| `enlaces.json` | Enlaces guardados en el gestor | Se crea vacío al arrancar |
| `config.json` | Preferencias (tema, logos, **modo ligero**, volumen, descargas, navegador de cookies, calidad YouTube/Twitch/Kick, buffer IPTV, subtítulos, recordar lista, avisos de versión, URL de EPG), geometría de ventanas, última lista lateral y canal (sin autoplay), cola e historial de YouTube, historial IPTV / Twitch / Kick, últimas URLs de **Archivo → Descargar** | Se crea con valores por defecto al arrancar. Se edita en **Archivo → Preferencias** |
| `cookies.txt` | Cookies de YouTube | Se escribe al reproducir YouTube o al pulsar **Reexportar cookies**, solo si hay login vigente en el navegador. El indicador **Sesión YouTube: OK / caducada** avisa si hace falta reexportar. **No va al git** (`.gitignore`). Detalle en [YouTube](youtube.md#cookies) |
| `twitch_cookies.txt` | Cookies de Twitch | Se escribe con **Twitch → Reexportar cookies** si hay login vigente en el navegador. El reproductor y el menú **Twitch** muestran **Sesión Twitch: OK / caducada** (igual que YouTube). Sirve para directos o VOD restringidos (p. ej. solo suscriptores). **No va al git** (`.gitignore`). |
...
`app_version.py` guarda el número (p. ej. 1.2.4). `app_update.py` consulta `https://api.github.com/repos/entreunosyceros/kidneysm3u/releases/latest` (User-Agent `kidneysm3u/…`) y extrae `x.y.z` del tag o del título (`Versión1.2.4`, `v1.2.4`, etc.). Al arrancar, si `check_app_updates` es verdadero, como mucho una vez cada 24 h (`app_update_checked_at` + `app_update_cache` en `config.json`). **Ayuda → Buscar actualizaciones** ignora esa caché y consulta ya. No se registran las URLs de descarga.
