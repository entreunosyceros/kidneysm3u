...
Menú **Kick → Cargar URL de Kick**: pega un enlace de canal en directo (`https://kick.com/canal`), un VOD (`…/videos/…`) o un clip. También puedes pegar una URL de Kick en **Reproducir → Cargar URL** y se abrirá como Kick sin ir al menú. El programa usa **yt-dlp** para obtener el stream HLS y lo reproduce con VLC embebido. La calidad máxima se elige en **Preferencias → Calidad Kick** (360 / 720 / 1080 / mejor). Los directos no muestran barra de progreso; los VOD sí. Los **VOD** guardan la posición al cambiar de canal o cerrar y se reanudan al volver a abrirlos. En directos, si caduca el enlace HLS, el reproductor vuelve a pedir el stream con yt-dlp. Si abres un **canal offline**, como en Twitch puedes elegir **reproducir el último VOD**, abrir el canal en el navegador o cancelar. **Kick → Añadir a favoritos** guarda el canal o VOD actual. **Kick → Recientes** y **Reproducir → Historial** listan las últimas URLs abiertas. **Kick → Reexportar cookies** (o **Preferencias → Cookies**) guarda `kick_cookies.txt` desde el navegador; ayuda con VOD restringidos o errores **403** de Cloudflare. Instalar **curl-cffi** en el `.venv` (`pip install curl-cffi`) mejora mucho la extracción de VOD. El menú **Kick** y la pestaña **Cookies** muestran **Sesión Kick: OK / caducada**. La búsqueda integrada en Kick no está incluida en esta versión.
