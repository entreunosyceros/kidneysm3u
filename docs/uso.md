...
- **Historial** — IPTV, YouTube, Twitch y Kick (recientes y **seguir viendo**). **Ver historial…** o **Biblioteca…** abren la biblioteca unificada con filtros por fuente.
...
- **Cargar URL** — directo (`https://www.twitch.tv/canal`), VOD o clip (también desde **Reproducir → Cargar URL**).
- **Buscar…** — canales en directo, offline y VOD recientes.
- **VODs del canal…** — lista VOD y aviso si está en directo.
- **Ver chat…** (tecla `C`) — chat del directo en ventana flotante.
- Favoritos, recientes, reexportar cookies y calidad en Preferencias.

Si el canal está offline, puedes abrir el navegador o el último VOD. Los VOD guardan la posición. Detalle (cookies, chat, `pywebview`): más abajo en este mismo apartado y en [instalación](instalacion.md).

Menú **Twitch → Cargar URL de Twitch**: pega un enlace de canal en directo (`https://www.twitch.tv/canal`), un VOD (`…/videos/…`) o un clip. También puedes pegar una URL de Twitch en **Reproducir → Cargar URL** y se abrirá como Twitch sin ir al menú. El programa usa **yt-dlp** para obtener el stream y lo reproduce con VLC embebido. Al cargar un **canal**, comprueba si está **en directo** o **offline**; si no emite, puedes abrir la página en el navegador o reproducir el último VOD. La calidad máxima se elige en **Preferencias → Calidad Twitch** (360 / 720 / 1080 / mejor). Los directos no muestran barra de progreso; los VOD sí. Los **VOD** guardan la posición al cambiar de canal o cerrar (como YouTube/IPTV) y se reanudan al volver a abrirlos. En directos, si caduca el enlace HLS (~30–60 min), el reproductor vuelve a pedir el stream con yt-dlp (como el watchdog de IPTV). Al cargar verás canal, título, si está en directo o VOD, calidad y si usa cookies. Las URLs recientes aparecen en **Reproducir → Historial** y en **Favoritos → Biblioteca…**.

**Twitch → Añadir a favoritos** guarda el canal o VOD que estés viendo en `favoritos.json` (igual que IPTV/YouTube/Kick). **Twitch → Ver chat…** abre el chat del directo en una ventana flotante (solo emisiones **en vivo**, no VOD ni clips). También puedes pulsar **C** con un directo en marcha. En **Preferencias → Reproducción** puedes activar **Abrir chat al iniciar un directo de Twitch**. La librería `pywebview` se instala con el resto de dependencias al arrancar con `run_app.py`. En Linux hacen falta además `python3-gi`, `gir1.2-gtk-3.0` y `gir1.2-webkit2-4.1` (véase [instalación](instalacion.md)). Si el `.venv` usa otro Python (p. ej. Conda), el chat puede abrirse con el Python del sistema. **Twitch → Buscar…** consulta Twitch (canales en directo, canales offline y VOD recientes) y reproduce el resultado en el reproductor con doble clic. **Twitch → VODs del canal…** pide el nombre del canal, comprueba si **está en directo** (con botón para verlo al momento) y lista los **VOD recientes** para reproducirlos con doble clic. **Twitch → Recientes** lista las últimas emisiones abiertas (también en **Reproducir → Historial**). **Twitch → Reexportar cookies** (o **Archivo → Preferencias → Cookies**) guarda tu sesión de Twitch desde el navegador en `twitch_cookies.txt`. Sirve para emisiones restringidas (p. ej. solo suscriptores). Usa el mismo **Navegador de cookies** de Preferencias (en Windows, lo fiable es **Firefox**). El estado **Sesión Twitch: OK / caducada** y el botón **Reexportar cookies** están en la pestaña **Cookies** de Preferencias; el menú **Twitch** también muestra el estado.

## Kick

Menú **Kick**:

- **Cargar URL** — directo (`https://kick.com/canal`), VOD o clip (también desde **Reproducir → Cargar URL**).
- **Buscar…** — canales; si el término es un slug, también VOD recientes de ese canal.
- **VODs del canal…** — lista VOD y aviso si está en directo.
- Favoritos, recientes, cookies y calidad en Preferencias (`curl-cffi` ayuda con VOD/Cloudflare).

Si el canal está offline, puedes abrir el navegador o el último VOD. Los VOD guardan la posición.

Menú **Kick → Cargar URL de Kick**: pega un enlace de canal en directo (`https://kick.com/canal`), un VOD (`…/videos/…`) o un clip. También puedes pegar una URL de Kick en **Reproducir → Cargar URL** y se abrirá como Kick sin ir al menú. El programa usa **yt-dlp** para obtener el stream HLS y lo reproduce con VLC embebido. La calidad máxima se elige en **Preferencias → Calidad Kick** (360 / 720 / 1080 / mejor). Los directos no muestran barra de progreso; los VOD sí. Los **VOD** guardan la posición al cambiar de canal o cerrar y se reanudan al volver a abrirlos. En directos, si caduca el enlace HLS, el reproductor vuelve a pedir el stream con yt-dlp. Si abres un **canal offline**, como en Twitch puedes elegir **reproducir el último VOD**, abrir el canal en el navegador o cancelar. **Kick → Buscar…** consulta Kick (canales en directo u offline) y, si el término coincide con un slug de canal, también lista **VOD recientes** de ese canal; doble clic reproduce en el reproductor. **Kick → VODs del canal…** pide el nombre del canal, comprueba si **está en directo** (con botón para verlo al momento) y lista los **VOD recientes**. **Kick → Añadir a favoritos** guarda el canal o VOD actual. **Kick → Recientes** y **Reproducir → Historial** listan las últimas URLs abiertas (también en **Favoritos → Biblioteca…**). **Kick → Reexportar cookies** (o **Preferencias → Cookies**) guarda `kick_cookies.txt` desde el navegador; ayuda con VOD restringidos o errores **403** de Cloudflare. Instalar **curl-cffi** en el `.venv` (`pip install curl-cffi`) mejora mucho la extracción de VOD. El menú **Kick** y la pestaña **Cookies** muestran **Sesión Kick: OK / caducada**.
